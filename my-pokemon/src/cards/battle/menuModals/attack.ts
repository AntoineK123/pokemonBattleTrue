//cette function va render une modale qui presentera les attaques disponibles d'un pokemon
//
import { DivMenuStyle } from "../../../utils";
import type { menuElement } from "../../../interfaces";
import { renderBattleCard } from "../battle";
import { PokeofNewBattle, toggleTurn, turn } from "../../../data/data";
import type { Pokemon } from "../../../classes/classes";



export function renderAttackModal(divtoOverwrite: HTMLElement) {

    //on cree une div vierge
    const attackModal = document.createElement("div");

    //on prepare le menuele array 
    const menuEleArray: menuElement[] = [];

    //on demarre une boucle qui va créér un menueleArr :
    const currentPoke = PokeofNewBattle[turn.playersId] as Pokemon

    currentPoke.attacksArr.forEach((attack, index) => {

        menuEleArray.push(
            {
                label: `${attack.label} (pp:${attack.pp})`,
                "selected": index === 0 ? true : false,
                onClickFunc: (() => {
                    //si il n'y a plus de pp alors on return
                    if (attack.pp === 0) {
                        return
                    }
                    currentPoke.attack(turn.opponent, index);
                    //on réduit le pp de l'attaque (le nb d'attaques restantes)
                    currentPoke.attacksArr[index].pp += -1
                    //on change le turn pour le prochain tour
                    toggleTurn();
                    renderBattleCard();
                }) as any
            })




    });

    //on ajoute un bouton retour à l'element array
    menuEleArray.push({
        label: "Retour",
        selected: false,
        onClickFunc: (() => {
            renderBattleCard();
        })
    })

    //fonction qui va creer le menu et l'append à l'attackmodal
    DivMenuStyle(menuEleArray, attackModal);
    //on overwrtie la div modale précedente pour la nouvelle avec la liste des attaques
    divtoOverwrite.innerHTML = "";
    divtoOverwrite.append(attackModal);
    return


}
